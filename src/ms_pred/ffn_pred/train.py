...
def add_ffn_train_args(parser):
    ...
    parser.add_argument(
        "--loss-fn", default="cosine", action="store", choices=["mse", "cosine"]
    )
...
def train_model():
    ...
    logging.info(
        f"Train, val, test of sizes: {len(train_dataset)}, {len(val_dataset)}, {len(test_dataset)}"
    )
